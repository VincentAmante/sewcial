...
const bubbleColour = computed(() => {
  const clrOverride = props.colourOverride.toLowerCase()
...
const alignmentStyle = computed(() => {
  const leftStyling = {
    ...
    arrow: ['border-l-[50px]', 'border-l-transparent', '-transform-y-2', 'translate-x-[-2.5em]', 'rotate-[-15deg]', 'left-0', 'border-b-accent-1', bubbleColour.value.arrow]
  }
  const rightStyling = {
